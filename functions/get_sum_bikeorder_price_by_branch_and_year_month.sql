...
drop function if exists Sales.BikeOrderSumByBranch
go

create function Sales.BikeOrderSumByBranch (@BranchName varchar(50), @Year smallint, @Month tinyint=null)
returns money
as
begin
	...
	if @Month is null
		begin
			--- Just year
			select @result= sum(TotalCost) from Sales.BikeOrderHeader
							where OrderDate between @YearStart and @YearEnd
								  and BikeOrderHeaderId=@BranchFilter
		end
